...
export default function Bilan() {
  ...
  return (
    <div>
      {/* Titre */}
      <div className="mb-6">
        <h2 className="text-xl font-bold text-slate-800 tracking-tight">Bilan de la classe</h2>
        <p className="text-slate-400 text-sm mt-1">{total} étudiant(s) au total</p>
      </div>
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="flex flex-col gap-4 min-w-32">
            <div className="text-center p-4 bg-blue-50 rounded-xl border border-blue-100">
              <div className="text-2xl font-bold text-blue-600">{stats.admis}</div>
              <div className="text-xs text-slate-500 mt-1">Admis</div>
              <div className="text-xs text-blue-400 font-medium">
                {total > 0 ? ((parseInt(stats.admis) / total) * 100).toFixed(0) : 0}%
              </div>
            </div>
            <div className="text-center p-4 bg-orange-50 rounded-xl border border-orange-100">
              <div className="text-2xl font-bold text-orange-500">{stats.redoublants}</div>
              <div className="text-xs text-slate-500 mt-1">Redoublants</div>
              <div className="text-xs text-orange-400 font-medium">
                {total > 0 ? ((parseInt(stats.redoublants) / total) * 100).toFixed(0) : 0}%
              </div>
            </div>
          </div>
        </div>
